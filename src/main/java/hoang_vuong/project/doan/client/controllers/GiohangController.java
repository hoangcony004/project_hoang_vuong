...
public class GiohangController {
    ...
    public ResponseEntity<Object> postGioHangThemMoi(Model model,
        ...
        data.put("success", "Đã thêm " + ten + " vào giỏ hàng");
        data.put("total", demSanPhamTrongGioHang());// tạm thôi
        // Nếu mà thất bại trong việc thêm vào giỏ hàng
        // data.put("redirect", "đường dẫn đến thông tin sản phẩm");//
        // "/product-info.php?product_id={$_POST['product_id']}";
    ...
    public String getGioHangChiTiet(Model model, RedirectAttributes redirectAttributes, HttpSession session) {
        // Nếu không có sản phẩm trong giỏ hàng, chuyển hướng về trang chủ
        if (!gioHangCoSanPham()) {
        ...
        if (cartMap != null) {
            for (Integer maSanPham : cartMap.keySet()) {
                SanPham sp = dvlSanPham.xem(maSanPham);
                String donGiaStr = String.valueOf(sp.getDonGia());
                float thanhTien = cartMap.get(maSanPham) * sp.getDonGia();
        ...
        // Trả về layout-client.html với content là trang giỏ hàng
    ...
    public ResponseEntity<Object> postGioHangSuaAjax(Model model,
        ...
        if// nếu sản phẩm đã có trong giỏ hàng, thì tiến hành cập nhật số lượng mới của nó
          // cũng như của toaàn giỏ hàng
        (cartMap.containsKey(id)) {
    ...
    public ResponseEntity<Object> postGioHangXoaAjax(Model model,
        ...
        if// nếu sản phẩm đã có trong giỏ hàng, thì tiến hành gỡ bỏ và cập nhật lại thông
          // tin số lượng
        (cartMap.containsKey(id)) {
            int so_luong_cu = cartMap.get(id);
            cartMap.remove(id);
